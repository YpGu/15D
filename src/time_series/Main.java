...
public class Main
{
	...
	calcObj(List<Long> votingMatPos, List<Long> votingMatNeg) {
		...
		for (long pbid: votingMatPos) {
			...
			for (int k = 0; k < K; k++) 
				prob += p[pid][k] * q[bid][k];
			res += Math.log(logis(prob) + Double.MIN_VALUE);
		}
		for (long pbid: votingMatNeg) {
			int pid = (int)(pbid / NUM_BILLS);
			int bid = (int)(pbid % NUM_BILLS);
			...
			for (int k = 0; k < K; k++) 
				prob += p[pid][k] * q[bid][k];
			res += Math.log(1 - logis(prob) + Double.MIN_VALUE);
		}
		return res;
	}
...
//			if (fileCount == 10000) break;
	...
	public static void main(String[] args) {
		...
		FileParser.output(p, "./res/p");
		FileParser.output(q, "./res/q");
		FileParser.output(b, "./res/b");
	}
}
